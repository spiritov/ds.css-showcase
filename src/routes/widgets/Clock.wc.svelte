...
>
	...
	<div class="absolute top-0 left-0 z-10 grid size-full place-content-center">
		<div class="relative flex size-[10px] items-end justify-center bg-(--color-ds-clock-center)">
			<div
				class="absolute bottom-0.5 -z-10 ml-[6px] h-12 w-1 origin-bottom bg-(--color-ds-clock-dot)"
				style:rotate={`${((date.getHours() % 12) / 12) + ((1 / 12) *  (date.getMinutes() / 60))}turn`}
			></div>
			<div
				class="absolute bottom-0.5 -z-10 ml-1.5 h-16 w-1 origin-bottom bg-(--color-ds-clock-dot)"
				style:rotate={`${date.getMinutes() / 60}turn`}
			></div>
			<div
				class="absolute bottom-0.5 -z-10 ml-1.5 h-18 w-1 origin-bottom bg-(--color)"
	...
	<!-- clock numbers -->
	<div
		class="absolute top-0 left-1 flex size-full flex-col items-center justify-between text-[40px] leading-none tracking-tighter text-(--color-ds-clock-text)" style:font-family="var(--font-clock)"
	>
		<span class="pt-[10px]">12</span>
		<div class="flex w-full justify-between px-0.5">
			<span>9</span>
			<span class="pr-1">3</span>
		</div>
		<span class="pb-0.5">6</span>
	</div>
	<!-- clock number dots -->
...
{#snippet dotPair()}
	<span class="size-2 bg-(--color-ds-clock-dot)"></span>
	<span class="size-2 bg-(--color-ds-clock-dot)"></span>
{/snippet}
